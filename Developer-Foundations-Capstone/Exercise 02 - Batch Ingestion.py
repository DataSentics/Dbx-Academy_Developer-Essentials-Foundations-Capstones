...
check_data_frame19 = data_frame19.filter(col('ingested_at').isNull())
display(check_data_frame19)

# COMMAND ----------

display(dbutils.fs.ls("dbfs:/dbacademy/lucian-florin.bidica@datasentics.com/developer-foundations-capstone/batch_orders_dirty.delta"))
...
data_frame17.write.format('delta').mode("overwrite").save(batch_target_path)
...
data_frame18.write.format('delta').mode('append').save(batch_target_path)
data_frame18.count()

# COMMAND ----------

display(data_frame18)
data_frame18.printSchema()
...
data_frame19.write.format('delta').mode('append').save(batch_target_path)

# COMMAND ----------

display(data_frame19)
data_frame19.printSchema()
