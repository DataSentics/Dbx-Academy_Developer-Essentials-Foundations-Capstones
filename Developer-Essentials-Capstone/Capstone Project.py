...
def writeToBronze(sourceDataframe, bronzePath, streamName):
    (sourceDataframe
    .withColumn('eventDate',to_date(col("eventParams.client_event_time"),"MM-dd-yyyy"))
...
def loadStaticData(path):
    deviceLookupDF = (spark
                      .read
                      .format('delta')
                      .load(path)
                      .filter("deviceType == 'ios' OR deviceType == 'android'")
                     )
    return deviceLookupDF
...
def silverToGold(silverPath, goldPath, queryName):
    ...
    .groupBy("week")
    .agg(approx_count_distinct(col("device_id"),rsd=.01).alias("WAU"))
